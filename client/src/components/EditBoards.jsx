...
const EditBoards = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure selectedBoard is not null
      if (!selectedBoard) {
      ...
      const res = await API.put(`/user/${userId}/board/${selectedBoard.name}`, {
        newName: newName, // Send the new board name as part of the request body
      });
    ...
    >
      <div className="relative w-full max-w-md mx-auto dark:bg-gray-700 bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-lg font-semibold dark:text-white text-black">Edit Board</h3>
          <button
